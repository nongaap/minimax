...
describe('currentTurn function test', () => {
  ...
  it('Empty board returns X', () => {
    expect(
      fourbyfour.currentTurn(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']),
    ).to.equal('X');
  });

  it('Board returns X', () => {
    expect(
      fourbyfour.currentTurn(['X', '', '', '', '', '', 'O', '', '', '', '', 'X', 'O', '', '', '']),
    ).to.equal('X');
  });

  it('Board returns O', () => {
    expect(
      fourbyfour.currentTurn(['X', '', '', '', '', '', '', '', '', '', 'O', 'X', '', '', '', '']),
    ).to.equal('O');
  });
});
...
describe('gameProgress function test', () => {
  ...
  it('Empty board returns 0', () => {
    expect(
      fourbyfour.gameProgress(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']),
    ).to.equal(0);
  });

  it('Function returns total moves gven partial board', () => {
    expect(
      fourbyfour.gameProgress(['X', '', '', '', '', '', 'O', '', '', '', '', 'X', 'O', '', '', '']),
    ).to.equal(4);
  });
});
...
describe('fourbyfourAI function test', () => {
  ...
  it('Returns completed board when a completed board is passed in', () => {
    ...
    ).to.eql(['X', 'O', 'O', 'O', 'X', 'X', 'X', 'O', 'O', 'X', 'O', 'X', 'X', 'O', 'X', 'O']);
  });
  ...
  it('fourbyfourAI results in a tie when the function plays itself', function () {
    ...
    const expected = true;
    const actual =
      !winningCombo(input, fourbyfour.winningCombinations, ['X', 'O']) && input.indexOf('') === -1;
